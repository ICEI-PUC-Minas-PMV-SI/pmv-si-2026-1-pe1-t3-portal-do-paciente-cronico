...
async def run_test():
    ...
    try:
        ...
        # -> Fill CPF and password for patient João and submit the login form (click 'Entrar').
        ...
        # -> Open the Perfil page to find the logout (Sair) control.
        # link "Perfil"
        elem = page.locator("xpath=/html/body/nav/a[4]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the 'Sair da Conta' (logout) button to sign out, then verify the login screen is shown (look for 'Entrar' or CPF field).
