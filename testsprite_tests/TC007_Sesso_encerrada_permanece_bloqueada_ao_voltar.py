...
async def run_test():
    ...
    try:
        ...
        # -> Fill CPF and password fields and submit the login form.
        ...
        # -> Abrir a página 'Perfil' (clicar no item Perfil) para localizar o botão de 'Sair' e realizar logout.
        # link "Perfil"
        elem = page.locator("xpath=/html/body/nav/a[4]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click 'Sair da Conta' to log out, wait for the UI to update to the login screen, then press browser Back and verify the authenticated area does not return and the login screen remains displayed.
