...
async def run_test():
    ...
    try:
        ...
        # -> Fill the CPF field with the patient's CPF, fill the password, and submit the login form.
        ...
        # -> Click the mark-as-taken button for the first medication (Losartana), wait for the UI to update, reload the dashboard, then collect button elements to verify the taken state persists.
        # button
        ...
        # -> Reload the dashboard page and verify that Losartana remains marked as taken for today by collecting the medication buttons/state.
        await page.goto("http://localhost:8080/pages/dashboard.html")
        try:
            await page.wait_for_load_state("domcontentloaded", timeout=5000)
        except Exception:
